...
def _stage1_digest(client, corpus):
    ...
    prompt = (
        ...
        "환산해 'MM/DD' 형태로 바꿔 쓰세요 (예: 전일 작성 메시지의 '오늘 코스닥 급등' → "
        "'06/11 코스닥 급등'). 어느 날인지 판단이 안 되면 작성시각을 그대로 병기하세요.\n"
        "- ★원인 보존(최우선): 가격·지수·섹터가 움직인 '이유'(특정 기업 실적·가이던스, "
        "정책·금리 발표, 인물 발언, 수급 이벤트, 지정학 뉴스 등)가 원문에 있으면 결과 수치와 "
        "반드시 한 묶음으로 보존하세요. '반도체 급등'처럼 결과만 남기지 말고 "
        "'○○ 실적 서프라이즈 → 반도체 급등'처럼 원인→결과 형태로 쓰세요. 요약하면서 "
        "원인을 잘라내는 것이 가장 나쁜 정제입니다.\n"
        "- 각 주제는 가능하면 '무슨 일이 있었나(결과) ← 왜(원인) ← 그래서(채널의 해석)' "
        "구조로 정리하세요. 원문에 원인이 없으면 억지로 만들지 말고 '(원인 언급 없음)'이라고 "
        "표기하세요.\n"
...
def analyze_messages(messages, channel_name: str = "",
    ...
    prompt = (
        ...
        f"보고서 작성일(오늘)은 {today}입니다. 모든 '오늘·전일·간밤' 표현은 이 날짜 기준입니다.\n"
        ...
        "작성 철학:\n"
        "- ★날짜 구분: 채널 메시지는 전일 오후~오늘 아침에 걸쳐 수집되어, 메시지 속 '오늘'이 "
        "실제로는 전일을 가리키는 경우가 많습니다. 보고서에서는 반드시 작성일 기준으로 통일하고, "
        "'오늘·전일(어제)·간밤'을 각각 처음 언급할 때 날짜를 병기하세요 "
        "(예: '전일(06/11) 코스닥 +4.76% 급등', '간밤(06/11 미국장) 반도체 +7.9%', "
        "'오늘(06/12, 금) 갭업 출발 가능성'). 이후 같은 표현 반복 시에는 병기 생략 가능. "
        "어느 날 일인지 확실치 않은 사건은 단정하지 말고 시점을 모호하지 않게 쓰세요.\n"
        "- ★인과 사슬(가장 중요): 주요 변동(지수·환율·섹터·종목의 급등락)을 언급할 때는 "
        "반드시 '무엇이 그것을 촉발했는가(원인)'를 채널 메시지·뉴스 헤드라인에서 찾아 함께 "
        "쓰세요. '미국 증시가 상승했다' 같은 표면 서술로 끝내는 것을 금지합니다. "
        "'원인 → 결과 → 국내 시장 파급 경로' 사슬로 한 단계 이상 파고드세요 "
        "(예: '○○사 실적 서프라이즈와 AI 수요 전망 상향 → 필라델피아 반도체 +7.9% → "
        "국내 반도체 소부장 갭업 기대'). 입력 자료 어디에도 원인이 없으면 지어내지 말고 "
        ...
        "수치를 3개 이상 나열했는데 원인 설명이 하나도 없다면 잘못 쓴 문단입니다.\n"
