...
_NAVER_INTEG = "https://m.stock.naver.com/api/stock/{code}/integration"   # 추정PER·EPS(JSON)
_NAVER_ITEM = "https://finance.naver.com/item/main.naver?code={code}"     # HTML 폴백


def _naver_est(code):
    """네이버 증권 추정PER(컨센서스)·EPS → {'est_per': float, 'eps': float}. 실패 시 {}.

    1차: m.stock API(totalInfos JSON) — code/한글 라벨 둘 다로 매칭(스키마 변화 대비).
    2차: PC 종목페이지 HTML(_cns_per/_eps id → 라벨 근접 정규식) 폴백.
    'N/A'·빈값은 스킵. EPS는 원 단위(음수 가능), 추정PER은 0<x<10000만 채택.
    """
    c = _digits(code, 6)
    if len(c) != 6:
        return {}
    out = {}
    try:
        r = requests.get(_NAVER_INTEG.format(code=c), headers=_UA, timeout=8)
        if r.ok:
            for it in (r.json() or {}).get("totalInfos") or []:
                cd = str(it.get("code") or "")
                keyname = str(it.get("key") or "")
                val = _f(re.sub(r"[^\d.\-]", "", str(it.get("value") or "")))
                if val is None:
                    continue
                if (cd == "cnsPer" or "추정PER" in keyname) and 0 < val < 10000:
                    out.setdefault("est_per", val)
                elif (cd == "eps" or keyname == "EPS") and "추정" not in keyname:
                    out.setdefault("eps", val)
    except Exception:
        pass
    if "est_per" in out and "eps" in out:
        return out
    try:  # HTML 폴백
        r = requests.get(_NAVER_ITEM.format(code=c), headers=_UA, timeout=8)
        r.raise_for_status()
        t = r.text
        if "est_per" not in out:
            m = (re.search(r'id="_cns_per"[^>]*>\s*([\d,.\-]+)', t)
                 or re.search(r"추정PER.{0,400}?([\d][\d,]*(?:\.\d+)?)(?:\s|<[^>]*>)*배", t, re.S))
            v = _f(m.group(1)) if m else None
            if v and 0 < v < 10000:
                out["est_per"] = v
        if "eps" not in out:
            m = re.search(r'id="_eps"[^>]*>\s*([\d,.\-]+)', t)
            v = _f(m.group(1)) if m else None
            if v is not None:
                out["eps"] = v
    except Exception:
...
def collect() -> dict:
    ...
    for x in cands:
        ...
        recent.append({
            ...
            # 네이버 컨센서스 — 아래 후처리에서 채움
            "est_per": None, "eps": "", "eps_won": None,
        ...
        if dk and cc:
            ...
            if raw:
                ...
                if lk:
                    ...
                    n_lock += 1
        # ── 네이버 컨센서스(추정PER)·EPS ──
        ne = _naver_est(r.get("code"))
        if ne.get("est_per") is not None:
            r["est_per"] = round(float(ne["est_per"]), 1)
            n_nv += 1
        if ne.get("eps") is not None:
            r["eps_won"] = float(ne["eps"])
            r["eps"] = f"{int(round(ne['eps'])):,}"
        time.sleep(0.08)
        r.pop("_lstg", None)
        r.pop("_cc", None)
    ...
    _log(f"완료: 최근상장 {len(recent)}종목 "
         f"(crno {n_crno} · 상장일 {n_listed} · 섹터 {n_sect} · 재무 {n_fin} · 공모가 {n_ipop} · 추정PER {n_nv} · 회사소개 {n_intro} · 보호예수 {n_lock} · 추이 {n_spark}) "
         f"/ 향후 {len(upcoming)}건")
