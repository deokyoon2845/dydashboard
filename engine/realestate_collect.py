...
_RENT_DISABLED = False  # 전월세(getRTMSDataSvcAptRent) 미승인 감지 시 True
#   → 해당 런에서 전월세 호출을 건너뛰고 '매매-only'로 폴백(전세 등락·전세가율만 제외).
#   매매(상세자료)와 전월세는 data.go.kr에서 '별도' 활용신청이라 한쪽만 승인된 경우가 흔하다.
#   진단/수집이 전월세 AUTH(rcode 30/31/32)를 만나면 죽지 않고 이 플래그만 세운다.
...
def diagnose(asof=None, test_code=_DIAG_CODE):
    ...
    매매(상세자료)와 전월세(전·월세 자료)를 '둘 다' 점검한다 — 둘은 별도 활용신청이라
    한쪽만 승인된 경우가 흔하다. 매매가 막히면 치명(수집 불가)으로 다루고, 전월세만
    미승인이면 _RENT_DISABLED를 세워 '매매-only'로 폴백(수집은 OK로 진행)한다.
    성공 시 동작 scheme(https/http)을 _SCHEME에 캐시한다.
    """
    global _SCHEME, _RENT_DISABLED
    _RENT_DISABLED = False
    key = _get_key()
    if not key:
        return ("NO_KEY",
    ...
    net_err = None
    empty_detail = None
    sale_state = None          # "OK" | "EMPTY" | None(아직 미확정)
    sale_note = ""

    # ── 1) 매매(상세자료) — 동작 scheme 탐색까지 겸함 ──────────────
    for scheme in _SCHEMES:
        ...
        for ym in yms:
            ...
            if info["kind"] == "OK":
                _SCHEME = scheme
                sale_state = "OK"
                sale_note = f"강남구 {ym} 매매 {len(info['items'])}건({scheme})"
                break
            if info["kind"] == "AUTH":
                code = info.get("rcode")
        ...
        if scheme_ok:
            _SCHEME = scheme
            sale_state = "EMPTY"
            sale_note = f"강남구 최근 2개월 매매 표본 0건({empty_detail})"
            break
    ...
    # ── 2) 전월세(전·월세 자료) — 별도 활용신청 대상. 동작 scheme로 1콜만 ──
    rent_msg = ""
    try:
        rres = _request(key, test_code, yms[0], "전월세", _scheme())
        rinfo = _interpret(rres)
        if rinfo["kind"] in ("OK", "EMPTY"):
            rent_msg = "전월세 정상"
        elif rinfo["kind"] == "AUTH" and rinfo.get("rcode") in ("30", "31", "32"):
            _RENT_DISABLED = True
            rent_msg = ("전월세 미승인 → 전세 등락·전세가율 빼고 매매만 수집합니다. "
                        "전세까지 보려면 data.go.kr에서 '국토교통부_아파트 전월세 실거래가 자료'"
                        "(getRTMSDataSvcAptRent) 활용신청을 추가하세요")
        else:
            rent_msg = f"전월세 확인 보류({rinfo.get('kind')})"
    except Exception as e:
        rent_msg = f"전월세 확인 보류(네트워크: {str(e)[:50]})"

    if sale_state == "OK":
        return ("OK", f"정상 — {sale_note} · {rent_msg}. 수집을 진행할 수 있어요.")
    return ("OK_EMPTY",
            f"호출은 정상인데 {sale_note} · {rent_msg}. "
...
def _fetch(key, code, ym, trade_type, stats=None):
    ...
    if info["kind"] == "AUTH":
        global _RENT_DISABLED
        code = info.get("rcode")
        # 전월세만 미승인이면 죽지 않고 '매매-only'로 폴백(플래그만 세우고 건너뜀).
        if trade_type == "전월세" and code in ("30", "31", "32"):
            if not _RENT_DISABLED and stats is not None:
                stats["last_err"] = (f"전월세 미승인(rcode {code}) → 전세 지표 제외, "
                                     "매매만 수집")
            _RENT_DISABLED = True
            return []
        # 그 외(매매 미승인 등)는 모든 호출에서 동일 → 즉시 중단.
        raise RuntimeError(
            f"실거래 키 인증 오류 — {trade_type} 자료({_RTMS_PATH[trade_type]}) "
            f"미승인/무효 (data.go.kr 활용신청·승인 상태 확인). 원문: {info['msg']}")
