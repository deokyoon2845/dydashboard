...
시세·차트 데이터 경로 (★pykrx 제거 — KRX가 클라우드 IP를 영구 차단하기 때문):
  ...
  · 종목 뉴스                 : 네이버 뉴스 검색 오픈API(NAVER_CLIENT_ID/SECRET)
                                · 폴백 finance.naver.com 종목뉴스 스크래핑(키 불필요)
...
def _naver_creds():
    """네이버 오픈API 자격증명. st.secrets 우선, 없으면 환경변수. 둘 다 없으면 ('','')."""
    cid = csec = ""
    try:
    ...
    if not csec:
        csec = os.environ.get("NAVER_CLIENT_SECRET", "") or ""
    return cid, csec


def _clean_news_title(t: str) -> str:
    """<b> 등 태그 제거 + &quot; 등 엔티티 복원."""
    t = re.sub(r"<[^>]+>", "", t or "")
...
def _parse_pubdate(s: str):
    """네이버 오픈API pubDate(RFC1123, 예: 'Mon, 22 Jun 2026 14:30:00 +0900') → KST datetime. 실패 시 None."""
    try:
...
def _src_from_url(u: str) -> str:
    """원문 링크 도메인을 출처 라벨로(예: hankyung.com). 네이버 호스트면 빈 문자열."""
    try:
        host = urlparse(u or "").netloc.lower()
        if host.startswith("www."):
            host = host[4:]
        if not host or "naver.com" in host:
            return ""
        return host
    except Exception:
        return ""
...
def _naver_news_search(query: str, display: int = 50):
    """네이버 뉴스 검색 오픈API → [{'title','url','source','dt'}] (최신순 raw).
    자격증명 없음/요청 실패 시 None(폴백 신호). 성공 시 리스트(빈 리스트 가능)."""
    ...
    try:
        r = requests.get(
            ...
            params={"query": query, "display": max(10, min(100, display)),
                    "sort": "date"},
            timeout=6)
        r.raise_for_status()
        items = (r.json() or {}).get("items", []) or []
    ...
    out = []
    for it in items:
        title = _clean_news_title(it.get("title", ""))
        link = (it.get("originallink") or it.get("link") or "").strip()
        dt = _parse_pubdate(it.get("pubDate", ""))
        if not title or not link or dt is None:
            continue
        out.append({"title": title, "url": link,
                    "source": _src_from_url(it.get("originallink") or link),
                    "dt": dt})
    return out


def _scrape_finance_news(code: str):
    """[폴백] finance.naver.com 종목뉴스 스크래핑(키 불필요).
    윈도우 필터 전 raw 리스트 반환 · 실패 시 []."""
    if not code:
        return []
    collected, seen = [], set()
    for page in (1, 2):
...
def _stock_news(code: str, name: str):
    """전영업일 장마감(15:30)~현재 사이 종목 뉴스 중
       제목에 종목명을 담은(관련도) → 최신순 상위 3개.
    1순위: 네이버 뉴스 검색 오픈API(NAVER_CLIENT_ID/SECRET).
    폴백  : finance.naver.com 종목뉴스 스크래핑(키 불필요).
    ...
    items = _naver_news_search(name, display=50)   # 오픈API 우선
    if items is None:                              # 자격증명 없음/오류 → 스크래핑 폴백
        items = _scrape_finance_news(code)
    if not items:
        return []

    seen, uniq = set(), []
    for it in items:
        tkey = re.sub(r"\s+", "", it["title"])[:40]
        if tkey in seen:
            continue
        seen.add(tkey)
        uniq.append(it)

    in_win = [c for c in uniq if start <= c["dt"] <= end]
    nrm = _norm(name)
