...
.mxwrap{border:1px solid var(--line);border-radius:12px;overflow-y:auto;max-height:360px;}
...
@media(max-width:760px){.maparea{height:420px}.nav{width:116px}.mrow{flex-direction:column}.mleft,.mright{width:100%;flex:none}.mxwrap{max-height:300px}}
</style></head><body>
<div class="hd">
...
function pathsOf(r){
  if(SUDO.has(r)){
    ...
    return f.map(d=>({n:d.n,sl:d.sl,d:d.d,cx:d.cx,cy:d.cy,mm:d.mm,js:d.js,v:d.v,vc:d.vc,jr:d.jr}));}
...
function drawMap(){_paths=pathsOf(region);_vb=vbOf(_paths);const svg=document.getElementById("big");
  ...
  svg.querySelectorAll("path.dist").forEach(p=>{const d=_paths[+p.dataset.i];
    p.onmouseenter=()=>{p.style.stroke="#7E9A83";p.style.strokeWidth="1.6";pg.appendChild(p);
      ...
      tip.innerHTML=`<div style="font-weight:800;margin-bottom:4px">${d.n}</div><div class="muted">매매 <span class="${cls(d.mm)}">${fmtP(d.mm)}</span> · 전세 <span class="${cls(d.js)}">${fmtP(d.js)}</span></div>${vline}<div class="muted">전세가율 ${d.jr==null?"-":d.jr+"%"}</div>`;
...
function drawChart(){const tk=trendOf(region);const sale=sliceP(tk.sale),jeon=sliceP(tk.jeonse);
  ...
  if(all.length<2){svg.innerHTML='<text class="axt" x="310" y="110" text-anchor="middle">추이 데이터가 아직 없어요</text>';return;}
  const L=52,Rr=606,T0=14,B=188,W=Rr-L,Hh=B-T0,n=sale.length,wk=SUDO.has(region);
  ...
  svg.innerHTML=`<defs><linearGradient id="gA" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="#B65F5A" stop-opacity=".24"/><stop offset="1" stop-color="#B65F5A" stop-opacity="0"/></linearGradient></defs>`
    ...
    +xt+`<text class="axtitle" x="${((L+Rr)/2).toFixed(0)}" y="224" text-anchor="middle">시점(${wk?"주→월 환산":"월"})</text>`
...
function setRegion(k){if(!RNAME[k]||busy)return;busy=true;region=k;
  ...
  setTimeout(()=>{drawLegend();drawMap();drawTable();drawChart();updateNote();
...
document.querySelectorAll("#periodSeg button").forEach(b=>b.onclick=()=>{document.querySelectorAll("#periodSeg button").forEach(x=>x.classList.remove("on"));b.classList.add("on");period=b.dataset.p;drawChart();});
...
@media(max-width:680px){.core{grid-template-columns:1fr}.row{grid-template-columns:1.4fr 64px 80px}.r-spark{display:none}.price{grid-template-columns:1fr}}
</style></head><body><div class="box">
  <div class="cycle">
...
function priceCard(p){const med=p.med||[],mean=p.mean||[];
 ...
 return '<div class="pc"><div class="pc-top"><span class="pc-reg">'+p.lab+'</span>'
  +'<span class="pc-tag">아파트 매매 · 월간(KB)</span></div>'+big+meanLine+chart+'</div>';}
...
def _render_indicator_charts(data):
    ...
    # 새 레이아웃 기준 높이 산정(클리핑 방지) — 헤더 + (가격블록) + 핵심3 + 그룹 행들.
...
function rowsRK(list,limit){return list.slice(0,limit).map(function(c,i){
 ...
 return '<div class="rkrow'+(i===0?' top1':'')+'"><div class="rank">'+(i+1)+'</div>'
  +'<div class="nm">'+badge+c.apt+'<small>'+meta+'</small></div>'
...
function flat(){var all=CAP.slice().sort(function(a,b){return b.cap-a.cap;}).slice(0,10);
 document.getElementById("flat").innerHTML=all.length?all.map(function(c,i){
  return '<div class="fr'+(i===0?' top1':'')+'"><div class="rank">'+(i+1)+'</div>'
   +'<div class="nm"><span class="gu-badge">'+c.gu+'</span>'+c.apt+'<small style="font-weight:600;color:var(--muted)">'+c.units.toLocaleString()+'세대 · '+c.dong+'</small></div>'
   +'<div class="cap"><b>'+capFmt(c.cap)+'</b><small>최근 '+(c.peok||'—')+'</small></div></div>';}).join("")
...
def _render_cap_leaders():
    ...
    height = 110 + min(len(rows), 10) * 54 + 110 + 10 * 58 + 90
...
def render_realestate():
    ...
    with t_anom:
        st.markdown('<div class="accent-bar"></div>', unsafe_allow_html=True)
        st.title("주목 단지")
        st.caption("구별·그룹별 시가총액 상위 단지 + 최근 거래가 몰린 상승 단지 + "
                   "특이거래(신고가·급등락·거래량) · 국토부 실거래 기준 · 직거래 기본 제외")
        st_cap, st_gain, st_hot, st_anom = st.tabs(
            ["시총 상위", "상승률", "주목 단지", "특이거래"])
        with st_cap:
