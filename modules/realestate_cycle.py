...
# ── 종합 강도 v3 — 6지표(매수우위·주담대·M2·착공·전세가율·GDP) 가중 결합 ────
#   설계(2026-07 v3 확정 · v2에서 개편):
#   · 매수우위: 레벨 배제·순수 방향성(4주/12주 2단 시계) — 유지
#   · 주담대: 볼록성 −Δr6M/r(저금리일수록 동일 bp 효과 증폭) 70%
#            + 금리 레벨(절대 저금리=매수여력) 30% — 레벨 항 신설
#   · M2: YoY 레벨+방향 반반 — 유지, 유동성 논지 반영해 비중 15→18%
#   · 착공: 역방향. 30개월 래그(착공→입주 시차 · 현재 체감 공급) 70%
#          + 현재 착공(향후 공급 기대 채널) 30% — 래그 신설, 비중 10→14%
#   · 전세가율: 레벨 단독 '갭투자 연료 게이지' — 높음=잠재 투자수요(+1까지),
#              낮음=연료 소진이나 똘똘한1채 국면 과도 감점 방지 위해 ×0.5 반감
#              (최소 −0.5). 방향 항 삭제(매매가 상승기마다 역효과 나던 구조).
#   ★상수는 전부 아래 — 여기만 조정.
_V3_W = {"buy": 0.28, "mortgage": 0.22, "m2": 0.18,
         "starts": 0.14, "jr": 0.10, "gdp": 0.08}
_V3_NM = {"buy": "매수우위", "mortgage": "주담대", "m2": "M2",
          "starts": "착공", "jr": "전세가율", "gdp": "GDP"}
_V3_T = 0.15            # 국면 판정 임계(v3 점수 스케일 -1~+1)
_V3_GO = 1.00           # 게이지 양끝 흡수 범위
_MORT_SCALE = 0.12      # 주담대: |Δr6M/r|=0.12 에서 신호 포화
_MORT_LC, _MORT_LHW = 3.5, 1.5  # 주담대 레벨 중심 3.5%·반폭 1.5%p(2%→+1/5%→−1)
...
_ST_LAG = 30                    # 착공→입주 시차(개월) — 2.5년
_JR_C, _JR_HW = 55.0, 10.0      # 전세가율 레벨 중심·반폭
_JR_NEG = 0.5                   # 전세가율 음수(연료 소진) 반감 계수 → 최소 −0.5
...
def _v3_at(series, mb, per_month):
...
def _v3_signal(key, s, mb):
    ...
    if key == "mortgage":                 # 월간 · 볼록성 70% + 레벨 30%
        i = len(s) - 1 - mb
        if i - 6 < 0 or i >= len(s) or not s[i]:
            return None
        conv = _clip1(-((s[i] - s[i - 6]) / s[i]) / _MORT_SCALE)
        lev = _clip1((_MORT_LC - s[i]) / _MORT_LHW)   # 저금리=+ (매수여력)
        return 0.7 * conv + 0.3 * lev
    ...
    if key == "jr":                       # 월간 · 레벨 단독(갭투자 연료 게이지)
        v = _v3_at(s, mb, 1)              #   음수(연료 소진)는 반감 — 똘똘한1채
        if v is None:                     #   국면 과도 감점 방지(최소 −0.5)
            return None
        x = _clip1((v - _JR_C) / _JR_HW)
        return x if x >= 0 else x * _JR_NEG
    if key == "starts":                   # 월간 · 역방향(공급축소=+)
        cur = _v3_at(s, mb, 1)            #   30개월 래그(입주 프록시) 70%
        lag = _v3_at(s, mb + _ST_LAG, 1)  #   + 현재 착공(공급 기대) 30%
        if lag is not None and cur is not None:
            return 0.7 * _clip1(-lag / _ST_S) + 0.3 * _clip1(-cur / _ST_S)
        if cur is not None:               # 래그 미확보(수집 초기 구간) → 현재만
            return _clip1(-cur / _ST_S)
        return None
    if key == "gdp":                      # 분기 · 레벨
        v = _v3_at(s, mb // 3, 1)
...
def _v3_score_series(data, months=78):
    ...
    for mb in range(months):
        num = den = 0.0
        n = 0
        for k, w in _V3_W.items():
            sig = _v3_signal(k, by.get(k) or [], mb)
            if sig is not None:
                num += w * sig
                den += w
                n += 1
        scarr.append(round(num / den, 3) if (n >= 3 and den) else None)
    if scarr[0] is None:                  # 현재 시점 판정 불가 → v3 비활성(폴백)
        return [], []
    while scarr and scarr[-1] is None:    # 꼬리 결측 정리
        scarr.pop()
    chips = []
    for k in _V3_W:
        sig = _v3_signal(k, by.get(k) or [], 0)
        if sig is None:
            continue
        cls = "up" if sig > 0.05 else ("dn" if sig < -0.05 else "flat")
        chips.append({"nm": _V3_NM[k], "c": cls,
                      "v": f'{"+" if sig >= 0 else "−"}{abs(sig):.2f}',
                      "w": int(round(_V3_W[k] * 100))})
    return scarr, chips


# v3 산식·가중치 스펙 — 배지(details) 안 표 렌더용. 산식 상수를 바꾸면 여기도 동기화.
_V3_SPEC = [
    ("매수우위", "28%", "순수 방향성 — 4주 변화(÷6p)·12주 변화(÷12p) 반반 · 레벨 미사용"),
    ("주담대", "22%", "볼록성 −(Δr 6개월÷현재 r)÷0.12 <b>70%</b> + 금리 레벨(중심 3.5%·±1.5%p) "
     "<b>30%</b> — 저금리일수록 같은 인하 효과 증폭"),
    ("M2", "18%", "YoY 레벨(중심 4.5%·±1.5%p) 50% + 6개월 방향(÷1.5%p) 50%"),
    ("착공", "14%", "역방향 · 30개월 래그(착공→입주 시차 · 현재 체감 공급) <b>70%</b> + "
     "현재 착공(향후 공급 기대) <b>30%</b> — 주거용 YoY(3개월 평균)÷25%"),
    ("전세가율", "10%", "레벨 단독 '갭투자 연료 게이지' — (전세가율−55%)÷10%p · "
     "음수(연료 소진)는 ×0.5 반감(최소 −0.5) · 방향 항 삭제"),
    ("GDP", "8%", "전기비 레벨(중심 +0.25%·±0.5%p)"),
]


def _v3_spec_foot_row():
    """'종합 강도 v3 산식·가중치' 배지 — foot_row와 동일한 details 배지 골격에
    ...
    foot = ('<div style="margin-top:7px;padding-top:7px;border-top:1px dashed #E4E5DE">'
            '종합 강도 v3 = 6지표 신호(−1~+1)의 가중합 · 국면 임계 ±0.15 · '
            '결측 지표는 제외하고 가중 재정규화 · '
            '기준금리는 주담대와 중복이라 점수 제외(차트 병기만)</div>')
    return ('<div class="ui-fx ui-foot-row"><details class="ui-foot">'
            '<summary><span class="pill">종합 강도 v3 산식·가중치'
...
function renderCycle(){const order=["침체기","회복기","상승기","둔화기"];
 ...
 function cycScoreOld(mb){let sc=0,n=0;lead.forEach(function(m){const r=m.real||[];const off=mb*(m.cad==="month"?1:4);
  ...
  return n?sc/n:0;}
 // 종합 강도 v3: 파이썬에서 6지표(매수우위·주담대·M2·착공·전세가율·GDP) 가중 결합으로
 ...
 // 판정 근거 — v3면 6지표 가중 기여 칩(신호값·가중치), 아니면 구 선행지표 칩
 ...
 if(V3){
   ...
   whyLab='판정 근거 — 종합 강도 v3 · 6지표 가중(매수우위28·주담대22·M2 18·착공14·전세가율10·GDP 8)';
 ...
 document.getElementById("cycGauge").innerHTML=
   '<div class="cg-lab">현재 위치 — 임계 ±'+TS+' 기준 국면 판정'+(V3?' · v3':'')+' · 전체 궤적은 하단 거시 섹션</div>'
...
def _render_indicator_charts(data):
    ...
    # 종합 강도 v3 — 6지표 가중(파이썬 사전계산). 거시 미수집이면 빈 배열 → JS 구 방식 폴백.
    _scarr, _v3sig = _v3_score_series(data)
...
<div class="bt"><div class="bt-lab">종합 강도 궤적 — 2020.1~ 월별 v3 점수 · 서울 매매지수 비교(백테스트)</div>
...
def _render_macro_backtest(data, scarr):
    """종합 강도 v3 월별 궤적 vs 서울 매매지수 오버레이 — 가중치·임계 검증용.
...
def _render_macro_section(data, scarr=None):
    ...
    st.markdown('<div class="re-grp">거시 상세 추이'
                ...
                '종합 강도 v3 입력 지표 · 메인 상세</span></div>', unsafe_allow_html=True)
    ...
    with _c2:
        ...
        st.markdown(_v3_spec_foot_row(), unsafe_allow_html=True)
    ...
    st.markdown(foot_row(
        ...
        "십자선 호버로 시점별 값 확인 · 백테스트=종합 강도 v3를 2020년부터 소급 계산해 "
