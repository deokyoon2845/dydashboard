...
function renderCycle(){const order=["침체기","회복기","상승기","둔화기"];
 ...
 document.getElementById("cycGauge").innerHTML=
   ...
   +'<div class="cg-lab" style="margin-top:9px">현재 위치 — 임계 ±'+TS+' 기준 국면 판정'+(V2?' · v2':'')+'</div>'
...
<div class="bt"><div class="bt-lab">백테스트 — 종합 강도 v2(2020.1~) vs 서울 매매지수</div>
...
function draw(){
 ...
 var slo=Math.min.apply(null,SALE),shi=Math.max.apply(null,SALE);
 ...
 function path(a,Y){return 'M'+a.map(function(v,i){return X(i).toFixed(1)+','+Y(v).toFixed(1);}).join(' L');}
 var svg='<svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">'
  +band+yg
...
def _render_macro_backtest(data, scarr):
    ...
    import json as _json
    if not scarr or len(scarr) < 12:
        return
    by = {it.get("key"): it for it in (data or [])}
    sale = [float(v) for v in ((by.get("sale") or {}).get("series") or [])
            if v is not None]
    sc = [v for v in scarr if v is not None]
    m = len(sc)
    sale_m = []
    for mb in range(m):                    # 주간 매매지수 → 월 샘플링(4주 간격)
        i = len(sale) - 1 - mb * 4
        sale_m.append(sale[i] if 0 <= i < len(sale) else None)
    pairs = [(s, x) for s, x in zip(sc, sale_m) if x is not None]
    if len(pairs) < 12:
        return
    sc2 = [p[0] for p in pairs][::-1]      # 과거→현재
    sale2 = [round(p[1], 2) for p in pairs][::-1]
...
def _render_macro_section(data, scarr=None):
    ...
    st.markdown('<div class="re-grp">거시 상세 추이'
                ...
                '종합 강도 v2 입력 지표</span></div>', unsafe_allow_html=True)
