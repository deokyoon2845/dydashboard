...
# ── 종합 강도 v4 — 4지표 z-score 가중 결합 · lag 실측 반영(2026-07-14 확정) ────
#   설계 근거: engine/macro_lag_probe(2026-07-13~14 · 3회 실행) 실측.
#   타깃을 '매매지수 레벨'이 아닌 **3개월 변화율(모멘텀)**로 재정의하고, 지표별
#   lag 0~36개월 스캔 + 고정 스펙 조합 비교로 확정(조합 A · 경제해석형).
#   백테스트 성능: corr +0.869 · 방향 적중률 85% (2020.1~2026.7 · n=78).
#
#   파이프라인(지표 공통): 월간화 → 3개월 이동평균 → z-score(자체 창)
#                        → lag 시프트 → 부호×가중 합 → 가용 성분 가중 재정규화.
#   채택 지표(가중 = 실측 |corr| 비례):
#     매수우위(레벨−100)  lag 0  · 부호+ · 29%  [심리 · 동행 · corr +0.86]
#     착공 YoY(3M평균)    lag 29 · 부호− · 27%  [공급 시차 ≈2.5년 · corr −0.80]
#     전세지수 YoY        lag 0  · 부호+ · 23%  [전세→매매 전이 · corr +0.68]
#     주담대 6개월 변화    lag 15 · 부호− · 22%  [금리 충격 파급 · corr −0.66]
#   탈락: 전세가율(+0.38 · lag36 스캔 경계 인공물) · GDP(−0.39 약함) ·
#        M2(lag26 −0.83이나 20~22 단일 유동성 사이클 과적합 위험 → 보류).
#
#   점수 스케일: 합성치를 백테스트 창(78개월) 평균·표준편차로 자체 정규화 —
#     score = clip((합성 − μ) ÷ 2σ, ±1) → 0 = 2020년 이후 평균 국면,
#     임계 ±0.15 = ±0.3σ(통계적 의미 고정) · 게이지·차트 축은 v3와 동일 재사용.
_V4_DEF = [
    # (지표키, 표시명, 가중, 부호, lag개월)
    ("buy",      "매수우위",    0.29, +1, 0),
    ("jeonse",   "전세지수YoY", 0.23, +1, 0),
    ("starts",   "착공",       0.27, -1, 29),
    ("mortgage", "주담대Δ6M",  0.22, -1, 15),
]
_V4_T = 0.15           # 국면 판정 임계(점수 스케일 · ±0.3σ 상당) — JS(0.15)와 동기화
_V4_SIG_MULT = 2.0     # score = clip((합성−μ) ÷ (σ×이 값), ±1)
_V4_MONTHS = 78        # 백테스트 개월 수(2020.1~)
...
# ── v4 월간 시계열 유틸 — dict {YYYYMM: float} 기반(probe와 동일 프리미티브) ──
def _ym_add(ym, k):
...
def _v4_components(data):
    """indicators 리스트 → v4 성분 [(key, nm, w, sign, lag, zdict)].
    · 매수우위: 주간 → 월평균 → 레벨−100
    · 전세지수 YoY: jeonse_m(월간·장기 — 엔진 v4 백필) 우선, 없으면 주간 jeonse 폴백
      (폴백 시 YoY는 2021.1부터만 — 백테스트 초반이 비지만 '현재 판정'은 정상)
    ...
    buy = {k: v - 100.0 for k, v in _weekly_to_monthly(_vals("buy")).items()}
    ...
    raw = {"buy": buy, "jeonse": _d_yoy(jlev),
    ...
    for ym, v in mort.items():
        ...
        if p is not None:
            raw["mortgage"][ym] = v - p
    comps = []
    for key, nm, w, sign, lag in _V4_DEF:
...
def _v4_score_series(data, months=_V4_MONTHS):
    """월별 종합 강도 v4 배열(scarr[mb]=mb개월 전 점수 · 0=현재)과 현재 기여 칩.
    가용 성분만 가중 재정규화 · 시점별 성분 2개 미만이면 None(궤적 선 분절).
    성분 2개 미만 상시 or 현재 시점 판정 불가면 ([], []) — JS 구 방식 폴백."""
    comps = _v4_components(data)
    if len(comps) < 2:
        return [], []
    cur = _ym_now()
    rawarr = []
...
# v4 산식·가중치 스펙 — 배지(details) 안 표 렌더용. 산식 상수를 바꾸면 여기도 동기화.
_V4_SPEC = [
    ("매수우위", "29%", "매수우위지수 <b>레벨−100</b>(주간→월평균) · lag 0 · 부호 + — "
     "매매지수 3M 변화율과 실측 corr +0.86(동행 최강)"),
    ("착공", "27%", "주거용 착공 YoY(3개월 평균) · <b>lag 29개월</b>(착공→입주 공급 시차) · "
     "부호 − — 착공 급감은 약 2.5년 뒤 공급부족으로 가격에 + · corr −0.80"),
    ("전세지수", "23%", "KB 전세가격지수(월간) <b>전년동월비</b> · lag 0 · 부호 + — "
     "전세 상승이 매매를 밀어올리는 전이 경로 · corr +0.68"),
    ("주담대", "22%", "주담대 금리의 <b>6개월 변화(pp)</b> · <b>lag 15개월</b>"
     "(금리 충격 파급 시차) · 부호 − — 금리 급등은 약 1년여 뒤 가격 부담 · corr −0.66"),
]


def _v4_spec_foot_row():
    """'종합 강도 v4 산식·가중치' 배지 — foot_row와 동일한 details 배지 골격에
    ...
    foot = ('<div style="margin-top:7px;padding-top:7px;border-top:1px dashed #E4E5DE">'
            '종합 강도 v4 = 4지표를 <b>3개월 평활 → z-score → lag 시프트</b> 후 '
            '부호×가중 합산하고, 백테스트 창(2020.1~) 평균·표준편차로 정규화한 점수 '
            '(0 = 2020년 이후 평균 국면 · ±1 = ±2σ) · 국면 임계 ±0.15(≈0.3σ) · '
            '결측 성분은 제외하고 가중 재정규화 · lag·가중치·부호는 전부 실측 확정'
            '(macro_lag_probe 2026-07 · 타깃=서울 매매지수 3개월 변화율 · '
            '조합 corr +0.87 · 방향 적중률 85%) · 탈락 — 전세가율(스캔 경계 인공물) · '
            'GDP(약함) · M2(단일 유동성 사이클 과적합 위험 → 보류)</div>')
    return ('<div class="ui-fx ui-foot-row"><details class="ui-foot">'
            '<summary><span class="pill">종합 강도 v4 산식·가중치'
...
function renderCycle(){const order=["침체기","회복기","상승기","둔화기"];
 ...
 function cycScoreOld(mb){let sc=0,n=0;lead.forEach(function(m){const r=m.real||[];const off=mb*(m.cad==="month"?1:4);
  ...
  return n?sc/n:0;}
 // 종합 강도 v4: 파이썬에서 4지표(매수우위·착공·전세지수·주담대) z-가중·lag 반영으로
 // 사전계산한 SCARR(월별)을 사용. 미수집이면 SCARR=[] → 구 방식 자동 폴백.
 ...
 // 판정 근거 — v4면 4지표 z-가중 기여 칩(신호값·가중치·lag), 아니면 구 선행지표 칩
 ...
 if(V3){
   ...
   whyLab='판정 근거 — 종합 강도 v4 · 4지표 z-가중(매수우위29·착공27·전세지수23·주담대22) · lag 실측 반영';
 ...
 document.getElementById("cycGauge").innerHTML=
   '<div class="cg-lab">현재 위치 — 임계 ±'+TS+' 기준 국면 판정'+(V3?' · v4':'')+' · 전체 궤적은 하단 거시 섹션</div>'
 ...
 document.getElementById("cycRead").innerHTML=trajLine+(V3
   ?'국면 전환은 종합 강도가 임계 ±'+TS+'를 위아래로 넘을 때 신호가 나와요 — 4지표 기여는 위 판정 근거, 전체 궤적은 하단 거시 섹션 참고.'
...
def _render_indicator_charts(data):
    ...
    # 종합 강도 v4 — 4지표 z-가중·lag 반영(파이썬 사전계산). 미수집이면 빈 배열 → JS 구 방식 폴백.
    _scarr, _v3sig = _v4_score_series(data)
...
def _clip_2020(dates, vals):
    """'전체(2020~)' 표시용 — 장기 백필(2015~ · v4 lag 입력) 지표도 화면은
    2020년부터. YYYYMM·YYYYQn 모두 문자열 비교로 '2020' 이상만 통과
    (고정폭 연도 접두라 사전순 비교가 시간순과 일치)."""
    keep = [(d, v) for d, v in zip(dates, vals) if str(d) >= "2020"]
...
def _macro_chart_payload(data, months):
    ...
    for key, per_month, dec in spec:
        ...
        if months is None:                  # '전체(2020~)' — 장기 백필 표시 클립
            dates, vals = _clip_2020(dates, vals)
...
<div class="bt"><div class="bt-lab">종합 강도 궤적 — 2020.1~ 월별 v4 점수 · 서울 매매지수 3개월 변화율 비교(백테스트__CORR__)</div>
...
def _render_macro_backtest(data, scarr):
    """종합 강도 v4 월별 궤적 vs 서울 '매매지수 3개월 변화율' 오버레이(백테스트).

    v4 타깃 정의(레벨→모멘텀)와 같은 축으로 비교해야 산식·임계 검증이 성립한다 —
...
def _render_macro_section(data, scarr=None):
    ...
    st.markdown('<div class="re-grp">거시 상세 추이'
                ...
                '종합 강도 v4 입력·참고 지표 · 메인 상세</span></div>', unsafe_allow_html=True)
    ...
    with _c2:
        ...
        st.markdown(_v4_spec_foot_row(), unsafe_allow_html=True)
    ...
    st.markdown(foot_row(
        ...
        "십자선 호버로 시점별 값 확인 · 백테스트=종합 강도 v4를 2020년부터 소급 계산해 "
