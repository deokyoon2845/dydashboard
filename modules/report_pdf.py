...
def _first_sentences(text: str, n: int = 2) -> str:
    """문장 단위로 앞 n개만 '완전하게' 반환. 글자수로 자르지 않아 '…' 짤림이 없다."""
    text = str(text or "").strip()
    if not text:
        return ""
    parts = [p for p in re.split(r'(?<=[.!?。])\s+', text) if p.strip()]
    return " ".join(parts[:n]).strip() if parts else text


def _recover_detail(name: str, topics: list) -> str:
    """이미 저장된 보고서의 잘린 테마(…) 복원용 — 같은 제목의 topic.fact에서
    완전한 앞 2문장을 가져온다. (DB의 옛 테마는 글자수로 잘려 있을 수 있음)"""
    name = str(name or "").strip()
    for tp in (topics or []):
        if isinstance(tp, dict) and str(tp.get("title", "")).strip() == name:
            return _first_sentences(str(tp.get("fact", "")), 2)
    return ""
...
def _derive_themes_from_topics(data: dict) -> list:
    ...
    detail은 fact의 앞 2문장을 '완전하게' 쓴다 — 글자수 컷/'…' 없음(짤림 방지)."""
    ...
    for tp in topics:
        ...
        if not name:
            continue
        detail = _first_sentences(str(tp.get("fact", "")), 2)
        stocks = [str(s).strip() for s in (tp.get("stocks") or []) if str(s).strip()]
        derived.append({
            "name": name,
            "detail": detail,
...
def _render_themes(data: dict):
    ...
    if not themes:
        themes = _derive_themes_from_topics(data)
    ...
    for th in themes:
        name = th.get("name", "")
        detail = str(th.get("detail", "")).strip()
        # ★이미 저장된 보고서의 잘린 테마(…/...)는 topic 원문에서 완전한 문장으로 복원
        if detail.endswith("…") or detail.endswith("..."):
            recovered = _recover_detail(name, topics)
            if recovered:
                detail = recovered
