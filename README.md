...
| 서브탭 | 내용 | 주요 데이터 |
|--------|------|-------------|
| **지수** | 코스피·코스닥 현황, 수급 추세, 외국인·기관 순매수 상위 종목, 시장 폭(상승/하락 종목 비율), 섹터 등락, 공포·탐욕 지수·VIX·RSI, 미국·원자재·암호화폐 히트맵, 환율·금리·금리차, 경제 일정 캘린더 | yfinance, pykrx, CNN Fear&Greed |
...
| **IPO** | 최근 2년 신규상장(시총 5,000억 이상)·향후 IPO 일정. 종목별 PER·PBR·PSR과 매출·영업이익·당기순이익(DART 최근 연간) | 금융위(data.go.kr), DART |
...
| 서브탭 | 내용 | 주요 데이터 |
|--------|------|-------------|
| **지도** | 전국 아파트 가격지도 — 지역별 가격·변동을 지도 위에 표시 | KB 가격지수, 국토부 실거래 |
| **지표** | 부동산 사이클 위치, 선행/동행/수급·심리/펀더멘털 그룹별 지표, 카드별 신호·해석 | ECOS(한국은행), KB 등 |
| **주목단지** | 최근 거래가 몰린 상승 단지(세대수·시공사·소재지·59/84㎡ 실거래가) + 특이거래(신고가·신저가·급등락·거래량 급증). 국토부 실거래 기준, 직거래 기본 제외 | 국토부 실거래 |
| **분양** | 한국부동산원 청약홈 분양정보 — 청약 임박·진행 단지 우선 | 청약홈 |
| **키워드** | 부동산 뉴스 기반 키워드 분석 | 네이버 뉴스 API |
...
| 국내 증시·수급 | pykrx | 코스피·코스닥·외국인/기관 수급 |
...
위 다이어그램(이 대화 화면 참고)이 그 흐름이다: **데이터 소스 → 엔진(GitHub Actions) → Supabase DB → 뷰어(주식/부동산)**.

데이터 종류별로 Supabase 테이블이 나뉘어 있어 잡(job)끼리 충돌하지 않는다 — 예: `realestate_snapshots`(부동산), `ipo_snapshots`(IPO), `leaders`(주도주), 보고서 등.
...
| 워크플로 | 실행 시각(KST) | 하는 일 |
|----------|---------------|---------|
| 부동산 실거래 수집 | 매일 07:00 | KB 가격지수·국토부 실거래·청약홈 분양 수집 → DB |
| 증시 IPO 수집 | 매일 06:00 | 최근 2년 상장·향후 IPO 수집 → DB |
| 주도주 수집 | 평일 17:30 | 전종목 스캔·주도주 점수화 → DB (장마감 후) |
| 전략·시황 보고서 | **수동 전용** | 보고서·키워드 생성 + 예측 채점 + 텔레그램 발송 |
